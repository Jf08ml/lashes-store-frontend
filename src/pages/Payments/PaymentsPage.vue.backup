...
const sendOrder = async () => {
  ...
  const shippingCost = deliveryAddress.value.city && deliveryAddress.value.city.length >= 4
    ? deliveryAddress.value.city.toLowerCase() === "neiva"
      ? 0
      : 20000
    : 0;
  ...
  const orderData = {
    ...
    items: listItems.value.map(item => ({
      ...
      image: item.image,
      // ✅ CORREGIR: Convertir 'references' a 'selectedVariant.selections'
    ...
    discountAmount: cuponDiscount.value,
    shippingCost: shippingCost,
    total: totalPayment.value,
    deliveryType: deliveryType.value,
    paymentMethod: 'contraentrega',
    internalNotes: `Pedido online - Tipo de entrega: ${deliveryType.value}`
  };
